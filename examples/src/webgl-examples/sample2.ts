...
function main()
{
    const webgl = WebGLRenderer.init({ canvasId: "glcanvas1", contextId: "webgl" });
  ...
  `;
    ...
    const buffers1 = initBuffers1();

    setInterval(() =>
    {
        // Draw the scene
        drawScene(gl, programInfo, buffers, canvas);
        drawScene1(webgl, program, buffers1, canvas);
    }, 16);
}
...
function drawScene1(webgl: WebGLRenderer, programInfo: IWebGLRenderPipeline, buffers: {
...
// Draw the scene.
//
function drawScene(gl: WebGLRenderingContext, programInfo: {
